...
// ====================================================================
  ...
  //      .rst_n(rst_n)
  //    );
//功能：实现一个数据深度缓存，CUT_READY=0时，缓存满时还能接收实现数据流水不断。CUT_READY=1时，缓存满时不能接收数据间断输入输出
module sirv_gnrl_pipe_stage # (
  ...
  //当深度为1时，就绪信号可能与下一级的就绪相关，从而形成逻辑链。使用CUT_READY来控制它
  parameter CUT_READY = 0,
  parameter DP = 1,//DP值只能 0 和 1
  parameter DW = 32
) (
  input           i_vld, 
  output          i_rdy, 
  ...
  if(DP == 0) begin: dp_eq_0//{ pass through
      ...
      // The valid will be set when input handshaked //当输入握手时有效
      assign vld_set = i_vld & i_rdy;
      // The valid will be clr when output handshaked //当输出握手时，有效性将是clr
      ...
      if(CUT_READY == 1) begin:cut_ready//{
          // If cut ready, then only accept when stage is not full//缓存满时不能接收
          assign i_rdy = (~vld_r);
      end//}
      else begin:no_cut_ready//{
          // If not cut ready, then can accept when stage is not full or it is popping//缓存满时还能接收
