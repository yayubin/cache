...
//功能：输入总线1的外设地址分配，根据分配的地址分别接不同的外设模块
//      输入总线2 接sirv_flash_qspi_top外设模块
module e203_subsys_perips(
  ...
  //----------输出未知总线，源头来自输入总线1------------------------------------------------------------
 //输出在顶层自己接自己
 // .O12_BASE_ADDR       (32'h1100_0000),//  * SysPer    : 0x1100 0000 -- 0x11FF FFFF   
  ...
  //////////////////////////////////////////////////////////
  //---------对外接到sirv_aon_top模块，Always ON  (PMU,RTC,WatchDog，LCLKGEN)---
  //-----------输出总线，源头来自输入总线1-----------------------------------------------------
  ...
  //--------对外接到e203_subsys_mems模块---------------------------------------
  //---------对内接sirv_flash_qspi_top模块-----------------------------------------------------------
  //-----------输入总线2, 没有参与本模块地址分配-----------------------------------------------------
...
//------------------------------------------------------------------
 ...
 //------------------------------------------------------------------
  ...
  output uart1_irq,                
  output uart2_irq,                
 //-----高速时钟配置输出，接到e203_subsys_hclkgen模块------------------------------------------------------------
  output        pllbypass ,
  output        pll_RESET ,
  output        pll_ASLEEP ,
  output [1:0]  pll_OD,
...
*/
 ...
 localparam  DW  = `E203_ITCM_DATA_WIDTH;
 localparam  AW  = `E203_ITCM_ADDR_WIDTH;
 localparam  MW  = `E203_ITCM_WMSK_WIDTH;
 localparam  AW_LSB =3;
...
// wire  [`ADDR_LEN-1:0]   memory_cmd_addr  ;  
   wire  [`ALL_ADDR_LEN-1:0]   memory_cmd_addr  ;  
   //wire  [24-1:0]   memory_cmd_addr  ;  
...
(
  ...
  .cache_en (1'b0),
//................................
  .cpu_cmd_valid(qspi0_ro_icb_cmd_valid), // Handshake valid
  .cpu_cmd_ready(qspi0_ro_icb_cmd_ready), // Handshake ready
  .cpu_cmd_read (qspi0_ro_icb_cmd_read ),  // Read or write//1为读
  //.cpu_cmd_addr (qspi0_ro_icb_cmd_addr[AW-1:AW_LSB] ), 
 // .cpu_cmd_addr (qspi0_ro_icb_cmd_addr[AW-1:AW_LSB] ), 
// .cpu_cmd_addr (qspi0_ro_icb_cmd_addr[15:2] ), 
//  .cpu_cmd_addr ({8'd0,qspi0_ro_icb_cmd_addr[24-1:2],2'd0}), 
   .cpu_cmd_addr ({8'd0,qspi0_ro_icb_cmd_addr[24-1:0]}), 

  .cpu_cmd_wdata(qspi0_ro_icb_cmd_wdata), 
  .cpu_cmd_wmask(qspi0_ro_icb_cmd_wmask), 
  
  .cpu_rsp_valid(qspi0_ro_icb_rsp_valid), // Response valid 
  .cpu_rsp_ready(qspi0_ro_icb_rsp_ready), // Response ready
  .cpu_rsp_rdata(qspi0_ro_icb_rsp_rdata), 
//..................................
...
(
 ...
 .memory_cmd_read  (memory_cmd_read ),  // Read or write
  ...
  .memory_rsp_rdata (memory_rsp_rdata),
     ...
     .perips_flash_icb_cmd_valid(qspi0_icb_cmd_valid),//yyb
     .perips_flash_icb_cmd_ready(qspi0_icb_cmd_ready),
     .perips_flash_icb_cmd_addr (qspi0_icb_cmd_addr ),
     .perips_flash_icb_cmd_read (qspi0_icb_cmd_read ), 
     .perips_flash_icb_cmd_wdata(qspi0_icb_cmd_wdata),
     .perips_flash_icb_rsp_valid(qspi0_icb_rsp_valid),
     .perips_flash_icb_rsp_ready(qspi0_icb_rsp_ready),
     .perips_flash_icb_rsp_rdata(qspi0_icb_rsp_rdata),

    //.qspi1_flash_irq(qspi1_flash_irq), 
